...
public class ProjectileController : MonoBehaviour
{
 ...
 //   private GameObject fXSpawnerProjectileObjPrefab;
    ...
    void OnCollisionEnter(Collision collision) // when the bullet collides with an object
    {
        ...
        if (collision.transform.tag != "NoBulletCollision")
        {
            ...
            if (collision.transform.GetComponent<EnnemyController>() != null)
            {
                ...
                Debug.Log("EnnemyController ReceiveDamages");

                PlayImpactFXs(collision.transform.position);
            }

            if ((collision.transform.tag != "Player") && (collision.transform.GetComponent<EnnemyController>() == null
                || (collision.transform.GetComponent<EnnemyController>() && NoDestroyOnEnnemyCollision == false))
               )
            {
                Debug.Log("DestroySelf on "+ collision.transform.name);
    ...
    void DestroySelf(bool ByItself) // Destroy the bullet by itself
    {
        if (explodeByItself || _collision != null)
        {
            if (!ByItself && _collision != null)
            {
            ...
            {
                Debug.Log(transform.name+" PlayImpactFXs(transform.position)");
